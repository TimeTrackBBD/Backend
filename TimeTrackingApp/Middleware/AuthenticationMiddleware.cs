...
public class AuthenticationMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context, IUserRepository usersRepository, UsersConverter usersConverter)
    {
        ...
        {
            await BuildErrorResponse(context, StatusCodes.Status401Unauthorized, new ValidationError("Authentication", "User Unauthorized, please set an access"));
            return;
        }
        ...
        Result<User> dbUserResult = usersRepository.GetUserByUserName(loggedInUserResult.Value.UserName);

        if (dbUserResult.IsFailure)
        {
            if (dbUserResult.ValidationErrors.Any(error =>
                    error.ErrorMessage.Contains("Username Does Not Link To An Existing User")))
            {
                // create new user if not registered
                Result<User> savedUserResult = usersRepository.CreateUser(loggedInUserResult.Value);
